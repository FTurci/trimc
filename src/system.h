...
// template <class T> 
// void System<T>::gcmc(double beta,double mu,bool threebody){
// 	// compute energy
...
// 	else{
// 		printf("no cell system: aborting.")
// 		exit(0);
// 	}
...
// 	if (dice(this->gen)<0.5){
// 		// removal
// 		if (particles.size()>0){
// 			// select  a particle to be removed
// 			pick = (int)(dice(this->gen)*particles.size());
// 			particles.erase(particle.begin()+pick);

// 			energy = particle_energy(particles[pick],threebody);
// 			// compute the energy for the selected particle

// 				// acceptance
// 			if (dice(this->gen)<npart*exp(-beta*deltaU)/(zz*vol) ){
// 			// accept removal;
// 			}else{
// 			// reject remvoal
// 			}
...
// 		// acceptance
// 		if (dice(this->gen)<zz*vol*exp(-beta*(mu+deltaU))/(npart+1) ){
// 			// accept insertion;
// 		}else{
// 			// reject insertion
// 		}
...
double System<T>::particle_energy(const Particle &p, bool threebody){
	...
	std::vector<int> neighs(cells.maxParticles);
	std::vector<double> distances(cells.maxParticles);
	...
	for (int c=0; c<cells[cid].neighbours.size(); ++c){
		...
		for (int n=0; n<cells[cell].tally;++n){
			...
			if (i!=j){
				...
				double ee=this->interaction.twobody.at2(dr2);
				// temporary only short distance for repulsive interactions
				// if(dr2<1.)
					e+= ee;

				if (threebody){
					if(interaction.twobody.distance<2.5){
		...
		if (num_neighs>=2){
			...
			for(int n1=0;n1<num_neighs-1;n1++){
					...
					for(int n2=n1+1;n2<num_neighs;++n2){
						...
						if (triangle[0] > triangle[1]) std::swap (triangle[0],triangle[1]);
						if (triangle[1] > triangle[2]) std::swap(triangle[1],triangle[2]);
						if (triangle[0] > triangle[1]) std::swap(triangle[0],triangle[1]);
						...
						if (triangle[2]<3.0)//& triangle[0]>1.2)
						{ 
								...
								// printf(" tb  %g %g %g %g \n", triangle[0], triangle[1], triangle[2],tb);
								e+=tb;
								// printf("tp %g %g  \n",this->interaction.at(1.0,1.0,1.0),this->interaction.at(1.05,1.1,1.15)
									// ,

									// interaction.twobody.at(0.9)+interaction.twobody.at(1.0)+interaction.twobody.at(1.05)
									// );
